...
from database import get_session, User, Vocabulary, add_vocabulary_word, get_user_vocabulary, toggle_star_word, remove_vocabulary_word, get_starred_words
...
menu = st.sidebar.selectbox("Menu", ["Login", "Register", "My Settings", "Quiz me", "Revise my writing", "My Vocabulary List"]) #adds a dropdown menu on the sidebar 
 
# Session management
if "logged_in" not in st.session_state: 
    #session_state is a streamlit feature for storing app-wide variables
...
if menu == "Register":
    ...
    if st.button("Register"): #if register button is pressed
    ...
    if st.button("Login"):
        if authenticate_user(username, password):
            ...
            st.rerun()  # Rerun to show imported words immediately
        else:
            st.error("Invalid username or password. Please try again.")

if st.session_state.logged_in:
    st.sidebar.write(f"Logged in as {st.session_state.username}")
    if st.sidebar.button("Logout"): #log out button 
        st.session_state.logged_in = False
        st.session_state.username = None
...
if menu=="My Settings":
    if st.session_state.logged_in:
        st.header("Change your settings!")
    ...
    if st.session_state.logged_in:
        ...
        if not starred_words:
            ...
            if st.session_state.q_generated:
                student_response = st.text_area("Your Answer", "Your Answer Here...")
...
if menu=="Revise my writing":
    if st.session_state.logged_in:
        student_response = st.text_area("Student writing", "your ideas here....")
        if st.button("Submit and get feedback!"):
            emojiRain()
            response = client.chat.completions.create(
